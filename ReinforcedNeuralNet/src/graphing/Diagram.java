...
public class Diagram extends BasicGame
{
	...
	public static boolean outputValues(
			double[] inputs, double[] outputs, String xAxisName, String yAxisName,
			double xAxisOffset, double yAxisOffset, double xRange, double yRange, Color blue, String graphName)
	{
		Log.setVerbose(false);
		
		Line[] connectingLines = createLinesFromData(inputs, outputs, xAxisOffset, yAxisOffset, xRange, yRange, blue);
		
		Line[] lines = new Line[connectingLines.length+6];
		for(int i=0;i<connectingLines.length ;i++) {
			lines[i] = connectingLines[i];
		}
		...
		lines[lines.length-1] = xAxis;
		lines[lines.length-2] = yAxis;
		lines[lines.length-3] = arrX1;
		lines[lines.length-4] = arrX2;
		lines[lines.length-5] = arrY1;
		lines[lines.length-6] = arrY2;
		...
		t[8] = new Text( new Point( (ORIGIN.x + xAxis.p2.x)/4, ORIGIN.y-2*X_AXIS_LABEL_OFFSET) , graphName, blue); // 1st population
		...
		diagram = new Diagram("Simulation", lines, t, xAxisOffset, yAxisOffset, xRange, yRange);
